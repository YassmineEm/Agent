...
def _sync_to_agent(payload: dict):
    ...
    try:
        ...
        logger.warning(f"⚠️ Sync sql_agent échoué: {e}")
...
def on_sqlagent_delete(sender, instance, **kwargs):
    ...
    if remaining.exists() and chatbot.sql_enabled:
        ...
        try:
            ...
            logger.warning(f"⚠️ Suppression sql_agent échouée: {e}")
...
def on_chatbot_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        if resp.status_code not in (200, 404):
            resp.raise_for_status()
        logger.info(f"🗑️ Chatbot '{instance.name}' supprimé du sql_agent")
    except Exception as e:
        logger.warning(f"⚠️ Suppression sql_agent échouée pour '{instance.name}': {e}")